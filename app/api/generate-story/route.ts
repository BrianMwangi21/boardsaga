...
import type { GameEngineData, MoveAnalysis } from '@/lib/stockfish-client'
...
function validateMovesInStory(story: Story, moves: ParsedMove[], engineData?: MoveAnalysis[] | Record<string, MoveAnalysis>): ValidationResult {
  const issues: string[] = []
  const validMoveNumbers = new Set(moves.map(m => m.moveNumber))
  const validSanNotations = new Map(moves.map(m => [m.san, m.moveNumber]))
  const evaluations = Array.isArray(engineData) ? engineData : Object.values(engineData || {})
  ...
  story.chapters.forEach(chapter => {
    if (chapter.chessBoards && chapter.chessBoards.length > 0) {
      chapter.chessBoards.forEach((board) => {
        ...
        const engineMove = evaluations[board.moveNumber - 1]
        ...
        } else if (!engineMove && moves[board.moveNumber - 1] && board.san !== moves[board.moveNumber - 1].san) {
    ...
    if (chapter.keyMoveReferences && chapter.keyMoveReferences.length > 0) {
      chapter.keyMoveReferences.forEach((ref) => {
        ...
        const engineMove = evaluations[ref.moveNumber - 1]
        ...
        } else if (!engineMove && moves[ref.moveNumber - 1] && ref.san !== moves[ref.moveNumber - 1].san) {
...
function fixChessBoardsInStory(story: Story, moves: ParsedMove[], engineData?: MoveAnalysis[] | Record<string, MoveAnalysis>): Story {
  const evaluations = Array.isArray(engineData) ? engineData : Object.values(engineData || {})
