...
### Bug 1: Search Variable Name Error
...
### Bug 2: All Filters Not Working
**Issue**: All filters (search, date range, result, format) appeared broken. No stories would appear when filters were applied. Reset button didn't restore all stories.

**Root Cause**: In the `fetchStories` useEffect, line 113 set `setStories(data)` but never populated `allStories`. The filtering useEffect at line 125 attempted to filter `allStories` which was always empty `[]`, so it always returned 0 stories.

**Fix**: Changed line 113 from `setStories(data)` to `setAllStories(data)`. Added check in filtering useEffect to handle empty `allStories` gracefully.

**Verification**: User tested with console logs showing:
- Search "ca": 25 → 22 stories ✓
- Search "carlsen": 25 → 12 stories ✓
- Result "1/2-1/2": 25 → 2 stories ✓
- Reset: Restored all 25 stories ✓
